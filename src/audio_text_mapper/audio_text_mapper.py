...
class ATMapper:
    ...
    def get_startp(self, audio_path, audio_num, chunk_time):
        ...
        with audio as source:
            ...
            return self.chunk_find(recognized_chunk, self.startp)
    ...
    def recognize(self, audio_path, audio_num, dur, chunk_time):
        ...
        with audio as source:
            ...
            for time in range(fragm_step, dur + 1, fragm_step):
                ...
                recorded = rec.record(source, offset=start_time, duration=chunk_time)
                recognized = ''
                try:
                    recognized = rec.recognize_google(recorded, language='ru-RU')
                except Exception as e:
    ...
    def map_segment(self, start_pos, end_pos, start_time, end_time):
        ...
        return [self.Markup(word_num, start_time + (dur / syll_cnt[-1]) * syll_cnt[word_num - start_pos]) for word_num in range(start_pos, end_pos)]
    ...
    def make_mapping(self, chunk, audio_num, dur, endt):
            ...
            if endt == dur:
                shift = 1
                segment_markup = self.map_segment(self.startp, chunk_pos + len(chunk.text) + shift, self.last_chunk.tstart, dur)
                self.startp = chunk_pos + len(chunk.text) + shift
            else:
