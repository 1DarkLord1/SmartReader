...
class ATMapper:
    ...
    def recognize(self, audio_path, dur, chunk_time=5):
        ...
        with audio as source:
            for time in range(0, dur, chunk_time):
                ...
                chunk_words = list(
                    map(lambda word: word.lower(), rec.recognize_google(recorded, language="ru-RU").split(' ')))
    ...
    def make_mapping(self):
        ...
        for i in range(len(markup)):
            ...
            chunk_word_sec = [(wordnum, namedtuple('Time', ['audio_num', 'sec'])(audio_num, markup[i].tstart + (
                        wordnum - pos) * aver_speed))
